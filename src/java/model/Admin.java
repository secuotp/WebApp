...
public class Admin {
    ...
    public static ArrayList getAdmin(String site_id) throws ClassNotFoundException, SQLException {
        ...
        String cmd = "SELECT username, firstname, lastname, email, role_name FROM admin_site_user a, web_developer w, role r WHERE a.user_id = w.user_id AND a.role_id = r.role_id AND site_id = " + site_id;
    ...
    public static int addSiteAdmin(int user_id, String site_id_string, int role) throws ClassNotFoundException, SQLException {

        int site_id = Integer.parseInt(site_id_string);
        String cmd = "INSERT INTO admin_site_user (user_id, site_id, role_id) VALUES (?, ?, ?)";
        Connection con = ConnectionAgent.getInstance();
        PreparedStatement ps = con.prepareStatement(cmd);
        ps.setInt(1, user_id);
        ps.setInt(2, site_id);
